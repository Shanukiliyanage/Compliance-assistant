...
export function getRecommendationForControl(
  ...
  const rules = {
    ...
    "A.5.1": {
      ...
      PARTIALLY_COMPLIANT:
        `${orgName} should update the policy to match how the organization operates, ensure everyone receives it and acknowledges it, confirm it is consistently applied across teams, and set a regular review cycle with evidence of review and updates.`,
    },

    "A.5.2": {
      NOT_COMPLIANT:
        `${orgName} should define key information security responsibilities (governance, risk, access, incident handling, and supplier security) and assign named owners for each. Document responsibilities (for example a responsibility assignment matrix (RACI)) and communicate them to relevant staff, including how to request approvals, handle exceptions, and report incidents.`,
      PARTIALLY_COMPLIANT:
        `${orgName} should clarify remaining gaps and overlaps, confirm accountability for key tasks, define escalation paths, and ensure responsibilities documentation is up to date, accessible, and understood by all relevant roles with periodic refresh and evidence of communication.`,
    },

    "A.5.3": {
      NOT_COMPLIANT:
        `${orgName} should identify high-risk activities (for example payments, administrative changes, access approvals, and code releases) and enforce separation so no single person can approve and execute the same critical action (for example dual approval or independent review). Ensure staff understand security roles, reporting paths, and who to contact for security support.`,
      PARTIALLY_COMPLIANT:
        `${orgName} should expand segregation of duties to all critical processes, formalize approval workflows, periodically review exceptions and conflicts, and make security role awareness consistent through onboarding and refresh training with tracked completion.`,
    },

    "A.5.4": {
      NOT_COMPLIANT:
        `${orgName} should make managers accountable for enforcing key security practices (for example training completion, acceptable use, access approvals, and secure handling) and implement a clear process to log, investigate, and resolve reported security issues with deadlines, ownership, and escalation if delayed.`,
      PARTIALLY_COMPLIANT:
        `${orgName} should standardize how managers monitor and reinforce security (for example checklists and periodic reviews) and improve consistency by tracking actions to closure, recording evidence of fixes, and reviewing repeat issues to prevent recurrence.`,
    },

    "A.5.5": {
      NOT_COMPLIANT:
        `${orgName} should define which authorities, regulators, or law enforcement contacts may be needed for serious security incidents, maintain an up-to-date contact list, assign an owner and review cycle, and link it to the incident response plan.`,
      PARTIALLY_COMPLIANT:
        `${orgName} should keep the authority contact list current (owner and review cycle) and ensure relevant staff know when and how to engage authorities as part of incident escalation procedures.`,
    },

    "A.5.6": {
      NOT_COMPLIANT:
        `${orgName} should identify relevant external communities and sources (for example industry groups, computer emergency response team feeds, and vendor security advisories) and assign someone to monitor and share relevant updates.`,
      PARTIALLY_COMPLIANT:
        `${orgName} should make participation effective by defining what is monitored, how updates are triaged, and how learnings feed into risk management and control improvements.`,
    },

    "A.5.7": {
      NOT_COMPLIANT:
        `${orgName} should set up a basic threat intelligence process (sources and review frequency), assess relevance, and convert key items into actions (for example patching, alerts, user warnings, and risk updates).`,
      PARTIALLY_COMPLIANT:
        `${orgName} should improve threat intelligence by documenting the process, ensuring regular review, and keeping evidence of decisions and actions taken.`,
    },

    "A.5.8": {
      NOT_COMPLIANT:
        `${orgName} should embed information security in projects from the start (security requirements, architecture review, secure configuration, data classification, and access design) and perform a simple project security risk assessment before implementation, recording outcomes and required controls.`,
      PARTIALLY_COMPLIANT:
        `${orgName} should ensure security is applied to all projects consistently by defining clear security checkpoints in the project lifecycle, standardizing the risk assessment method, keeping records, and verifying required controls are implemented before go-live.`,
    },

    "A.5.9": {
      NOT_COMPLIANT:
        `${orgName} should create and maintain an inventory of important information and related assets (for example key systems, devices, applications, and data sets), assign owners, include criticality and storage location, and define a regular review cadence with evidence of review.`,
      PARTIALLY_COMPLIANT:
        `${orgName} should complete missing assets and owners, define minimum required fields, reconcile the inventory against reality periodically, and align updates with change management so the inventory stays accurate over time.`,
    },

    "A.5.10": {
      NOT_COMPLIANT:
        `${orgName} should create an Acceptable Use Policy (AUP) covering devices, email, internet, data handling, prohibited actions, and reporting requirements, publish it to staff, and implement a formal acknowledgement process during onboarding and periodic refresh with records.`,
      PARTIALLY_COMPLIANT:
        `${orgName} should update the Acceptable Use Policy (AUP) to reflect actual working practices (for example remote work, bring your own device (BYOD), and cloud tools), ensure it is consistently enforced, and ensure acknowledgements cover all staff and contractors and are renewed when policies change.`,
    },

    "A.5.11": {
      NOT_COMPLIANT:
        `${orgName} should implement a formal joiner, mover, and leaver process to recover organizational assets (for example devices, badges, keys, and storage media) and remove or adjust access promptly when people leave or change roles. Use a checklist with an owner and keep evidence that each step was completed.`,
      PARTIALLY_COMPLIANT:
        `${orgName} should make asset return and access removal consistent across teams by standardizing the checklist, integrating human resources and information technology triggers, recording completion for every case, and periodically reviewing records to confirm nothing is missed.`,
    },

    "A.5.12": {
      NOT_COMPLIANT:
        `${orgName} should define a simple information classification scheme (for example Public, Internal, Confidential, and Restricted), assign owners for key data sets, and document handling rules for storage, sharing, encryption, and retention. Train staff to apply the scheme consistently.`,
      PARTIALLY_COMPLIANT:
        `${orgName} should strengthen information classification by expanding coverage to all important information, aligning the scheme with real business and legal requirements, and checking consistency across teams and systems with periodic review and updates.`,
    },

    "A.5.13": {
      NOT_COMPLIANT:
        `${orgName} should implement information labelling aligned to the classification scheme (for example for documents, email, and storage locations) so users can recognize how information must be handled. Provide clear guidance on when and how to label information and what protections are required for each label.`,
      PARTIALLY_COMPLIANT:
        `${orgName} should improve labelling by making it consistent across key tools (for example email, documents, and file storage), reinforcing correct use through awareness, and introducing technical enforcement where feasible for high-risk information sharing.`,
    },

    "A.5.14": {
      NOT_COMPLIANT:
        `${orgName} should define approved methods for transferring information and implement protections to prevent data leakage, such as secure transfer tools, encryption, access controls, and approvals for high-risk transfers. Implement monitoring where appropriate to detect repeated leakage risks.`,
      PARTIALLY_COMPLIANT:
        `${orgName} should strengthen information transfer controls by standardizing protections across common channels (for example email, file sharing, cloud links, and removable media), tuning monitoring and data loss prevention (DLP) rules where appropriate, and reviewing repeated leakage patterns to drive improvements.`,
    },

    "A.5.15": {
      NOT_COMPLIANT:
        `${orgName} should implement access control based on least privilege and need-to-know so people only have access required for their job. This should include access approvals, role-based access where possible, and prompt removal of access when no longer needed.`,
      PARTIALLY_COMPLIANT:
        `${orgName} should tighten high-risk access and ensure access control rules apply consistently across all key systems, keeping evidence of approvals and periodic reviews to prevent accumulation of unnecessary privileges.`,
    },

    "A.5.16": {
      NOT_COMPLIANT:
        `${orgName} should implement identity lifecycle management for joiners, movers, and leavers, ensuring unique user identities, documented approvals, timely provisioning and deprovisioning, and periodic checks for orphaned accounts across systems (including software as a service (SaaS) systems).`,
      PARTIALLY_COMPLIANT:
        `${orgName} should strengthen identity management by standardizing processes across all systems, improving timeliness of account updates, reconciling accounts regularly to detect orphaned identities, and automating provisioning and deprovisioning where feasible.`,
    },

    "A.5.17": {
      NOT_COMPLIANT:
        `${orgName} should define and enforce rules for authentication information (for example passwords, personal identification numbers (PINs), and multi-factor authentication (MFA)), including minimum standards, secure storage and handling (no sharing), secure reset procedures, and protection of authentication secrets.`,
      PARTIALLY_COMPLIANT:
        `${orgName} should strengthen authentication practices by expanding multi-factor authentication (MFA) coverage for critical systems and privileged accounts, improving password and reset controls, and ensuring authentication rules are implemented consistently across all relevant systems.`,
    },

    "A.5.18": {
      NOT_COMPLIANT:
        `${orgName} should implement regular access reviews to confirm that user access rights remain appropriate, prioritizing privileged access and access to sensitive information. Record reviewers, decisions, and removal of unnecessary access with evidence.`,
      PARTIALLY_COMPLIANT:
        `${orgName} should improve access reviews by making them consistent across all key systems, using a repeatable schedule, tracking removals to completion, and reporting recurring access issues so they can be corrected.`,
    },

    "A.5.19": {
      NOT_COMPLIANT:
        `${orgName} should establish a supplier security management process for suppliers that can access information or systems: identify in-scope suppliers, assess security risks before onboarding, define required controls, control supplier access, and assign an owner for ongoing oversight.`,
      PARTIALLY_COMPLIANT:
        `${orgName} should strengthen supplier security management by applying a standard risk assessment consistently, tracking remediation actions, reviewing supplier access regularly, and reassessing suppliers when services or risks change.`,
    },

    "A.5.20": {
      NOT_COMPLIANT:
        `${orgName} should update supplier contracting to include minimum information security requirements, such as confidentiality, data protection, access controls, incident notification expectations, subcontractor controls, secure termination, and assurance or audit rights where appropriate.`,
      PARTIALLY_COMPLIANT:
        `${orgName} should improve supplier agreements by standardizing security clauses based on supplier risk, closing missing requirements in existing contracts, and maintaining a review cycle to keep security terms current.`,
    },

    "A.5.21": {
      NOT_COMPLIANT:
        `${orgName} should implement vendor security assessments for relevant information and communication technology suppliers (for example questionnaires and evidence reviews), set minimum security expectations, and require remediation before granting access to systems or data.`,
      PARTIALLY_COMPLIANT:
        `${orgName} should strengthen supply chain security by applying assessments consistently, performing deeper reviews for higher-risk suppliers, tracking remediation actions, and reassessing suppliers over time or after significant changes.`,
    },

    "A.5.22": {
      NOT_COMPLIANT:
        `${orgName} should implement periodic supplier reviews covering service performance and security, including incidents, access, and assurance evidence, and define actions or escalation when requirements are not met.`,
      PARTIALLY_COMPLIANT:
        `${orgName} should make supplier reviews consistent by setting a schedule, retaining evidence of reviews and decisions, and tracking follow-up actions to completion.`,
    },

    "A.5.23": {
      NOT_COMPLIANT:
        `${orgName} should assess and manage security risks for cloud services by documenting cloud use, defining shared responsibility expectations with providers, validating provider security capabilities, and implementing cloud governance such as secure configuration baselines, identity and access management, logging, monitoring, and incident handling.`,
      PARTIALLY_COMPLIANT:
        `${orgName} should strengthen cloud security by standardizing cloud risk assessments, improving configuration and access governance, and continuously monitoring cloud security posture and supplier assurance as services evolve.`,
    },

    "A.5.24": {
      NOT_COMPLIANT:
        `${orgName} should define and document an incident management approach, including roles and responsibilities, reporting channels, triage and escalation steps, communication procedures, and a consistent way to record incidents and actions taken. Ensure staff know how to report incidents.`,
      PARTIALLY_COMPLIANT:
        `${orgName} should improve incident management preparation by formalizing and testing the approach (for example tabletop exercises), ensuring procedures are followed consistently, maintaining complete incident records, and reviewing records for quality and trends.`,
    },

    "A.5.25": {
      NOT_COMPLIANT:
        `${orgName} should define event triage and classification (for example severity levels, criteria, and escalation rules) so security events are assessed consistently to determine whether they are incidents and what response is required. Keep evidence of classification decisions.`,
      PARTIALLY_COMPLIANT:
        `${orgName} should improve the timeliness and consistency of triage by standardizing criteria, ensuring timely escalation, and keeping records of classification decisions and escalations for trend analysis.`,
    },

    "A.5.26": {
      NOT_COMPLIANT:
        `${orgName} should define incident response steps (containment, eradication, recovery, and communications), assign owners, and keep evidence of actions taken for each incident so damage is limited and recovery is controlled.`,
      PARTIALLY_COMPLIANT:
        `${orgName} should strengthen incident response by standardizing playbooks for common incidents, ensuring documentation and approvals are captured, and validating readiness through exercises with tracked improvement actions.`,
    },

    "A.5.27": {
      NOT_COMPLIANT:
        `${orgName} should perform post-incident reviews to understand what happened, identify root causes and improvement actions, update controls and training to prevent recurrence, and track corrective actions to completion.`,
      PARTIALLY_COMPLIANT:
        `${orgName} should ensure reviews happen consistently for significant incidents, prioritize high-risk lessons learned, and verify corrective actions are implemented and effective, not just recorded.`,
    },

    "A.5.28": {
      NOT_COMPLIANT:
        `${orgName} should implement a root cause analysis (RCA) process for serious incidents and define evidence handling procedures for investigations, including collection, secure storage, integrity protection, retention, and chain of custody where required. Train responders on these practices.`,
      PARTIALLY_COMPLIANT:
        `${orgName} should improve root cause analysis (RCA) and evidence handling by applying procedures consistently, tracking actions to closure, validating effectiveness, protecting evidence from tampering, and clearly defining ownership and retention requirements.`,
    },

    "A.5.29": {
      NOT_COMPLIANT:
        `${orgName} should define secure procedures for operating during disruptions so information remains protected, including secure communications, controlled emergency access, and documented exceptions with approval.`,
      PARTIALLY_COMPLIANT:
        `${orgName} should strengthen information security during disruptions by validating procedures through continuity and incident exercises and documenting and approving emergency exceptions with evidence and follow-up actions.`,
    },

    "A.5.30": {
      NOT_COMPLIANT:
        `${orgName} should identify critical services and implement information and communication technology readiness for business continuity, including backups, recovery mechanisms, and clear continuity ownership. The plan should state target recovery objectives such as the Recovery Time Objective (the maximum acceptable time to restore a service) and the Recovery Point Objective (the maximum acceptable data loss measured in time).`,
      PARTIALLY_COMPLIANT:
        `${orgName} should improve continuity readiness by ensuring mechanisms cover all critical services, keeping a documented Business Continuity Plan (BCP) current, and testing recovery regularly (including restore tests) with documented results and tracked improvement actions. The plan should include recovery objectives such as the Recovery Time Objective (the maximum acceptable time to restore a service) and the Recovery Point Objective (the maximum acceptable data loss measured in time).`,
    },

    "A.5.31": {
      NOT_COMPLIANT:
        `${orgName} should maintain a compliance requirements register covering legal, regulatory, and contractual information security obligations, assign owners, define applicability, and map requirements to policies, controls, and evidence. Review the register regularly.`,
      PARTIALLY_COMPLIANT:
        `${orgName} should strengthen compliance management by keeping the requirements register up to date, reviewing changes regularly, and ensuring obligations are implemented consistently with evidence across all relevant systems and suppliers.`,
    },

    "A.5.32": {
      NOT_COMPLIANT:
        `${orgName} should define rules to protect intellectual property, including software licensing compliance and protection of internal documents, designs, and source code. Assign ownership and train staff on appropriate handling and sharing.`,
      PARTIALLY_COMPLIANT:
        `${orgName} should improve intellectual property protection by inventorying licensed software and key assets, checking compliance periodically, and strengthening controls for access and sharing of sensitive intellectual property.`,
    },

    "A.5.33": {
      NOT_COMPLIANT:
        `${orgName} should protect important records against loss, unauthorized change, or destruction by defining retention and integrity requirements, controlling access, implementing backups and recoverability, and preventing unauthorized modification.`,
      PARTIALLY_COMPLIANT:
        `${orgName} should extend record protection controls to all critical records, improve integrity and retention consistency, test recovery, and review retention and disposal practices regularly.`,
    },

    "A.5.34": {
      NOT_COMPLIANT:
        `${orgName} should implement measures to protect personal data in line with privacy and data protection requirements, including identifying where personally identifiable information is processed, applying access controls and secure processing, defining retention and deletion, managing supplier handling, and ensuring incident response covers personal data breaches.`,
      PARTIALLY_COMPLIANT:
        `${orgName} should strengthen personal data protection by completing data inventories and risk checks, applying controls consistently across all systems and suppliers, and keeping evidence of ongoing privacy compliance reviews.`,
    },

    "A.5.35": {
      NOT_COMPLIANT:
        `${orgName} should schedule independent reviews of information security (for example internal audit or external assessments) on a regular basis and when major changes occur, document findings, and track corrective actions with owners and deadlines to completion.`,
      PARTIALLY_COMPLIANT:
        `${orgName} should improve independent reviews by ensuring adequate scope and frequency, keeping evidence, and verifying that corrective actions are effective after implementation.`,
    },

    "A.5.36": {
      NOT_COMPLIANT:
        `${orgName} should implement compliance monitoring to check whether staff and processes follow information security policies, rules, and standards (for example spot checks, audits, and control monitoring), record non-compliance, and track fixes to closure with evidence.`,
      PARTIALLY_COMPLIANT:
        `${orgName} should make compliance checks consistent by retaining evidence of outcomes, tracking corrective actions to completion, and addressing repeat issues through root cause improvements and targeted awareness.`,
    },

    "A.5.37": {
      NOT_COMPLIANT:
        `${orgName} should document operating procedures for important systems and processes (for example access management, backups, change management, incident handling, and supplier onboarding), keep procedures accessible and version-controlled, and assign owners and review dates.`,
      PARTIALLY_COMPLIANT:
        `${orgName} should improve documented procedures by filling remaining gaps, keeping procedures updated as systems change, and periodically verifying procedures are followed and effective, with actions tracked when issues are found.`,
    },
